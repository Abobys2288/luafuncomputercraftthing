--[[
    CCOS Desktop v7 — fixed click through, start menu, drag
]]
...
D.nextWinId=1 D.dragWin=nil D.dragOX=0 D.dragOY=0 D.lastDragRect=nil
D.mouse={x=0,y=0} D.redrawPending=false
...
local function readF(p) if not p or not fs.exists(p) then return nil end local f=fs.open(p,"r") if not f then return nil end local c=f.readAll() f.close() return c end

function D.markDirty() D.redrawPending=true end
...
function D.createWindow(title,cx,cy,cw,ch)
    ...
    local w={id=id,title=title or "Window",cx=cx or 30,cy=cy or 20,cw=cw or 200,ch=ch or 120,
        ...
        onDraw=nil,onKey=nil,onClick=nil,appButtons=nil}
    D.windows[#D.windows+1]=w D.activeWin=w D.markDirty() return w
end
...
function D.bringToFront(w)
    for i,v in ipairs(D.windows) do if v.id==w.id then table.remove(D.windows,i) D.windows[#D.windows+1]=w D.activeWin=w D.markDirty() return end end
end
...
function D.drawDesktop()
    ...
    -- Start button
    local sbx,sby=2,by+2 R.drawButton(sbx,sby,54,16,D.startMenuOpen) R.drawText(sbx+4,sby+6,"Start",C.BLACK)
    ...
    -- Start menu (drawn last, on top)
    if D.startMenuOpen then
        local mw,mh=140,96 local my=sby-mh if my<1 then my=1 end
        R.fillRect(sbx,my,mw,mh,C.GRAY) R.drawW95Raised(sbx,my,mw,mh)
        R.fillRect(sbx+2,my+2,20,mh-4,C.DBLUE) R.drawText(sbx+3,my+30,"CC",C.WHITE)
        local items={{"File Manager","files"},{"Editor","edit"},{"Settings","settings"},{"Shell","shell"},{"Reboot","reboot"},{"Shutdown","shutdown"}}
        local iy=my+4
        for _,it in ipairs(items) do
            local ih2=D.mouse.x>=sbx+24 and D.mouse.x<sbx+mw-4 and D.mouse.y>=iy and D.mouse.y<iy+12
            if ih2 then R.fillRect(sbx+24,iy,mw-28,12,C.DBLUE) end
            R.drawText(sbx+28,iy+2,it[1],ih2 and C.WHITE or C.BLACK)
            iy=iy+14
        end
    end
    D.redrawPending=false D.lastDragRect=nil
end
...
function D.drawWindow(w)
    ...
    local cy=y+17 local ch=hh-19
    -- App buttons row
    if w.appButtons then
        local abx=x+2 for _,btn in ipairs(w.appButtons) do local bw=#btn.label+4 R.drawButton(abx,cy,bw,14,false) R.drawText(abx+2,cy+3,btn.label,C.BLACK) abx=abx+bw+2 end
        cy=cy+16 ch=ch-16
    end
    R.fillRect(x+2,cy-2,ww-4,ch,C.GRAY)
    if w.onDraw then pcall(w.onDraw,w,x+3,cy,ww-6,ch) end
end

-- Returns: "reboot"|"shutdown"|"files"|"edit"|"settings"|"shell" or nil
function D.handleClick(mx,my,btn)
    local by=R.h-D.taskbarH
    -- 1. Start menu (topmost, check first)
    if D.startMenuOpen then
        local sby=by+2 local mw,mh=140,96 local my=sby-mh if my<1 then my=1 end
        -- Check click inside menu
        if mx>=2 and mx<2+mw and my>=my and my<my+mh then
            local items={"files","edit","settings","shell","reboot","shutdown"} local iy=my+4
            ...
            return nil  -- clicked menu but not on item
        end
        D.startMenuOpen=false  -- clicked outside menu
        -- Don't return nil yet — fall through to check other things
    end
    -- 2. Start button
    if mx>=2 and mx<56 and my>=by+2 and my<by+18 then D.startMenuOpen=not D.startMenuOpen D.markDirty() return nil end
    ...
    if my>=by then local bx=60 for _,w in ipairs(D.windows) do local bw=math.min(100,R.w-bx-55) if bw<25 then break end
            if mx>=bx and mx<bx+bw and my>=by+3 and my<by+17 then
                if w.minimized then w.minimized=false w.visible=true D.bringToFront(w) D.markDirty()
                elseif D.activeWin and D.activeWin.id==w.id then w.minimized=true D.markDirty()
                else D.bringToFront(w) D.markDirty() end return nil end bx=bx+bw+2 end return nil end
    -- 4. Desktop icons
    local icons={{"Files","files"},{"Editor","edit"},{"Settings","settings"},{"Shell","shell"}}
    local iw,ih2=48,42 local cols=math.max(1,math.floor((R.w-10)/(iw+10)))
    for i,ic in ipairs(icons) do local col=(i-1)%cols local row=math.floor((i-1)/cols) local ix=8+col*(iw+10) local iy=8+row*(ih2+8)
        if mx>=ix-2 and mx<ix+iw+2 and my>=iy-2 and my<iy+ih2+2 then return ic[2] end end
    -- 5. Windows
    ...
    if w then
        ...
        -- App buttons
        if w.appButtons and my>=w.cy+17 and my<w.cy+31 then local abx=w.cx+2 for _,button in ipairs(w.appButtons) do local bw=#button.label+4
                if mx>=abx and mx<abx+bw then if button.onClick then pcall(button.onClick) end D.markDirty() return nil end abx=abx+bw+2 end end
        -- Resize
...
function D.handleDrag(mx,my)
    ...
    -- Clear old outline by redrawing bg + overlapping windows
    if D.lastDragRect then
        local r=D.lastDragRect R.fillRect(r.x,r.y,r.w,r.h,C.DESKTOP)
        for _,v in ipairs(D.windows) do if v.id~=w.id and v.visible and not v.minimized then
            if not (v.cx+v.cw<r.x or v.cx>r.x+r.w or v.cy+v.ch<r.y or v.cy>r.y+r.h) then D.drawWindow(v) end end end
    end
...
function D.handleMouseUp()
    if D.dragWin then local w=D.dragWin w.cx=D.mouse.x-D.dragOX w.cy=D.mouse.y-D.dragOY
        ...
        D.dragWin=nil D.lastDragRect=nil D.markDirty() end
...
function D.run()
    ...
    while running do
        ...
        if event=="mouse_click" then
            D.mouse.x=p2 D.mouse.y=p3
            local a=D.handleClick(p2,p3,p1)
            ...
            if D.redrawPending then D.drawDesktop() end
        elseif event=="mouse_drag" then D.mouse.x=p2 D.mouse.y=p3 D.handleDrag(p2,p3)
            -- No full redraw during drag
        elseif event=="mouse_up" then D.handleMouseUp() if D.redrawPending then D.drawDesktop() end
        elseif event=="key" then if p1==keys.q and D.startMenuOpen then D.startMenuOpen=false D.markDirty() else D.handleKey(p1,nil) end
...
function D.app_fm()
    ...
    w.appButtons={{label="Open",onClick=function() local it=items[sel] if not it then return end
        ...
        else D.app_editor(path=="/" and ("/"..it) or (path.."/"..it)) end D.markDirty() end},
        {label="Back",onClick=function() path=getDir(path) sel=1 scroll=0 refresh() D.markDirty() end},
    ...
    w.onKey=function(w,key,char)
        ...
        elseif key==keys.enter then local it=items[sel] if not it then return end
            if it==".." then path=getDir(path) sel=1 scroll=0 refresh()
            elseif it:sub(1,1)=="/" then local np=path=="/" and it or (path..it) if fs.isDir(np) then path=np sel=1 scroll=0 refresh() end
            else D.app_editor(path=="/" and ("/"..it) or (path.."/"..it)) end
    ...
    w.onClick=function(w,mx,my) local lh=math.floor((w.ch-20)/8) for i=1,lh do local idx=scroll+i local iy=34+(i-1)*8
            if my>=iy-1 and my<iy+8 then sel=idx local it=items[idx] if not it then return end
                if it==".." then path=getDir(path) sel=1 scroll=0 refresh()
                elseif it:sub(1,1)=="/" then local np=path=="/" and it or (path..it) if fs.isDir(np) then path=np sel=1 scroll=0 refresh() end
                else D.app_editor(path=="/" and ("/"..it) or (path.."/"..it)) end return end end end
end
...
function D.app_editor(filePath)
    ...
    w.onDraw=function(w,cx,cy,cw,ch)
        local eh=math.floor((ch-16)/8) for i=1,eh do R.drawText(cx+2,cy+(i-1)*8,lines[sY+i] or "",C.BLACK) end
        if cL>sY and cL<=sY+eh then local cy2=cy+(cL-sY-1)*8 local cx2=cx+(cC-1)*6
            if cx2>=cx and cx2<cx+cw then R.fillRect(cx2,cy2,6,8,C.DBLUE) local c=(lines[cL] or ""):sub(cC,cC) R.drawText(cx2,cy2,c=="" and " " or c,C.WHITE) end end
        R.fillRect(cx,cy+ch-12,cw,10,C.GRAY) R.drawText(cx+2,cy+ch-10,"Ln "..cL..(modified and " [mod]" or ""),C.BLACK) end
    w.onKey=function(w,key,char)
...
function D.app_shell()
    ...
    w.appButtons={{label="Run",onClick=function() local cmd=inputLine inputLine="" output[#output+1]="> "..cmd
            ...
            local ml=math.floor((w.ch-16)/8) if #output>ml then scrollY=#output-ml end D.markDirty() end},
        {label="Close",onClick=function() D.destroyWindow(w) end}}
